...
const setupLights = (scene, lightDefs) => {
  ...
  Object.entries(lightDefs).forEach(([name, def]) => {
    ...
    if (name !== "home") {
      ...
      if (["point_1", "point_2", "point_3", "point_4"].includes(name)) {
        pivot.visible = true;
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // initial camera at point 4 → then move to 5
    camera.position.set(
      camPointDefs.camPoint_4.x,
      camPointDefs.camPoint_4.y,
      camPointDefs.camPoint_4.z
    );
    controls.target.set(...lightDefs.point_4.position);
    controls.update();

    ["point_1", "point_2", "point_3", "point_4"].forEach((name) => {
      const pivot = diamondPivots[name];
      pivot.visible = true;
      pivot.scale.set(1, 1, 1);
      lights[name].intensity = lightDefs[name].intensity;
    });

    gsap.to(camera.position, {
      x: camPointDefs.camPoint_5.x,
      y: camPointDefs.camPoint_5.y,
      z: camPointDefs.camPoint_5.z,
      delay: 0.3,
      duration: 1,
      ease: "power2.inOut",
    });
    gsap.to(controls.target, {
      x: lightDefs.point_5.position[0],
      y: lightDefs.point_5.position[1],
      z: lightDefs.point_5.position[2],
      delay: 0.3,
      duration: 1,
      ease: "power2.inOut",
      onUpdate: () => controls.update(),
      onComplete: () => {
        const pivot5 = diamondPivots.point_5;
        pivot5.visible = true;
        gsap.to(lights.point_5, {
          intensity: lightDefs.point_5.intensity,
          duration: 1,
          ease: "power2.inOut",
        });
        gsap.to(pivot5.scale, {
          x: 1,
          y: 1,
          z: 1,
          duration: 1,
          ease: "power2.inOut",
          onComplete: () => {
            gsap.to(clickableDiamonds.point_5.scale, {
              x: 0.3,
              y: 0.3,
              z: 0.3,
              duration: 1,
    ...
    const onClick = (e) => {
      ...
      if (hits.length) {
        ...
        if (
          ["point_1", "point_2", "point_3", "point_4", "point_5"].includes(nm)
        ) {
          setShowModal(true);
    ...
    //初始 travel: lvl5 active
    gsap.set(
      lvlRefs.current.filter((r) => r),
      { opacity: 0.27 }
    );
    if (lvlRefs.current[4]) gsap.set(lvlRefs.current[4], { opacity: 1 });

    // handlers for lvl clicks now include lvl5 (index 4)
    const handlers = [];
    [0, 1, 2, 3, 4].forEach((i) => {
