...
export default function Loader({ setIsLoading }) {
  ...
  useEffect(() => {
    ...
    // 1) Initialisation via gsap.set
    for (let i = 1; i <= totalPaths; i++) {
      gsap.set(`.path_${i}`, { opacity: 0.3 });
    }
    gsap.set(textEl, { opacity: 0 }); // texte invisible au départ
    ...
    const onReady = () => {
      ...
      // 2) Timeline : paths puis texte en fade
      ...
      // fade-in du texte après les paths
      tl.to(
        textEl,
        {
          opacity: 0.7,
        ...
        `>+=0.1` // 0.1s après la fin du dernier path
      );
    };
